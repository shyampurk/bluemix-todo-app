...
def sessionexpirecheck_handler(details):
	...
	logging.info("ENTERED SESSIONEXPIRECHECK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	logging.info("EXITED SESSIONEXPIRECHECK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
...
def loggedincheck_handler(details):	
	global connection
	logging.info("ENTERED LOGGEDINCHECK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (int(user_state)!=0):
		...
		loggedin = False
	logging.info("EXITED LOGGEDINCHECK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
...
def getAddressBook_handler(details,channel):
	global connection
	logging.info("ENTERED GETADDRESSBOOK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):
		...
		if (session_expiry == True):
			...
			except Exception  as error_expirerecover_addbook:
				logging.debug("The error_expirerecover_addbook Exception is %s %s"%(error_expirerecover_addbook,type(error_expirerecover_addbook)))
...
def getTaskList_handler(details,channel):
	logging.info("ENTERED GETTASKLIST_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):
		try:	
			...
			try:
				...
				logging.debug("The error_dbwherfetchappop Exception is %s %s"%(error_dbwherfetchappop,type(error_dbwherfetchappop)))
		...
		logging.info("EXITED GETTASKLIST_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
		...
		if (session_expiry == True):
			...
			try:
				...
				dBop_update(tableName_expire,columnName_expire,conditionColumnName_expire,conditionColumnValue_expire,updatevalue_expire)
			except Exception  as error_expirerecover_tasklist:
				logging.debug("The error_expirerecover_tasklist Exception is %s %s"%(error_expirerecover_tasklist,type(error_expirerecover_tasklist)))
...
def getTaskDetails_handler(details,channel,task_id,start_index):
	logging.info("ENTERED GETTASKDETAILS_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):
		...
		logging.info("EXITED GETTASKDETAILS_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
		...
		if (session_expiry == True):
			...
			try:
				...
				dBop_update(tableName_expire,columnName_expire,conditionColumnName_expire,conditionColumnValue_expire,updatevalue_expire)
			except Exception  as error_expirerecover_taskdetails:
				logging.debug("The error_expirerecover_taskdetails Exception is %s %s"%(error_expirerecover_taskdetails,type(error_expirerecover_taskdetails)))
...
def addNewTask_handler(details,channel,task_name,task_desc):
	logging.info("ENTERED ADDNEWTASK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):	
		...
		logging.info("EXITED ADDNEWTASK_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
		...
		if (session_expiry == True):
			...
			try:
				...
				dBop_update(tableName_expire,columnName_expire,conditionColumnName_expire,conditionColumnValue_expire,updatevalue_expire)
			except Exception  as error_expirerecover_newtask:
				logging.debug("The error_expirerecover_newtask Exception is %s %s"%(error_expirerecover_newtask,type(error_expirerecover_newtask)))
...
def addComment_handler(details,channel,task_id,comment):
	logging.info("ENTERED ADDCOMMENT_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):	
		...
		if (session_expiry == True):
			...
			try:
				...
				dBop_update(tableName_expire,columnName_expire,conditionColumnName_expire,conditionColumnValue_expire,updatevalue_expire)
			except Exception  as error_expirerecover_comment:
				logging.debug("The error_expirerecover_comment Exception is %s %s"%(error_expirerecover_comment,type(error_expirerecover_comment)))
...
def updateTaskStatus_handler(details,channel,task_id,update_state):
	logging.info("ENTERED UPDATETASKSTATUS_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (session_expiry == False and loggedin == True):
		...
		publish_handler(channel,response_code,response_message,details,result)	
		logging.info("EXITED UPDATETASKSTATUS_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
		...
		if (session_expiry == True):
			...
			try:
				...
				logging.debug("The error_expirerecover_updatetask Exception is %s %s"%(error_expirerecover_updatetask,type(error_expirerecover_updatetask)))
...
def doLogin_handler(login,passd,channel):
	logging.info("ENTERED DOLOGIN_HANDLER FOR %s AT TIME %s",str(login),str(datetime.datetime.utcnow()))
	...
	logging.info("EXITED DOLOGIN_HANDLER FOR %s AT TIME %s",str(login),str(datetime.datetime.utcnow()))
...
def doLogout_handler(details,channel):
	logging.info("ENTERED DOLOGOUT_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
	...
	if (loggedin == True):	
		...
		publish_handler(channel,response_code,response_message,details,result)
		logging.info("EXITED DOLOGOUT_HANDLER FOR %s AT TIME %s",str(details['DISPLAY_NAME']),str(datetime.datetime.utcnow()))
