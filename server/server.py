...
def dBop_addNewTaskinsert(task_name,owner_id,task_desc,dateofcreate,lastupdate,noofcomment,status):
	global connection
	connectioncheck_handler()	
	upload_query = "INSERT INTO "+DatabaseSchema+".TASKTABLE VALUES (DEFAULT,\'"+str(task_name)+"\',\'"+str(owner_id)+"\',\'"+str(task_desc)+"\',\'"+str(dateofcreate)+"\',\'"+str(lastupdate)+"\',\'"+str(noofcomment)+"\',\'"+str(status)+"\')"
...
def sessionexpirecheck_handler(details,channel):
	...
	if (diff_minutes <=30):
		...
		try:
			tableName_expire = ""+DatabaseSchema+".USERTABLE"
...
def getTaskDetails_handler(details,channel,task_id,start_index):
	...
	wherefetch_tableName = ""+DatabaseSchema+".COMMENTTABLE"
	...
	if (session_expiry == False and loggedin == True):	
		try:
			...
			if(int(task_status['TASK_STATUS']) == 1):
				...
				try:
					upload_query = "INSERT INTO "+DatabaseSchema+".COMMENTTABLE VALUES (\'"+str(comment)+"\',\'"+str(dateofcreate)+"\',\'"+str(task_id)+"\',\'"+str(owner_id)+"\',\'"+str(order)+"\')"
...
def doLogin_handler(login,passd,channel):
	...
	try:
		login_query = "SELECT * FROM "+DatabaseSchema+".USERTABLE WHERE EMAIL = \'"+login+"\' AND PASSWORD = \'"+passd+"\'"
